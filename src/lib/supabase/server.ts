import 'server-only'
import { createServerClient } from '@supabase/ssr'
import { createClient } from '@supabase/supabase-js'
import { cookies } from 'next/headers'

// ── Env guard — fail fast with clear error at startup ────────────
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
const SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY

if (!SUPABASE_URL) {
  throw new Error('[ZENITH] Missing env var: NEXT_PUBLIC_SUPABASE_URL')
}
if (!SUPABASE_ANON_KEY) {
  throw new Error('[ZENITH] Missing env var: NEXT_PUBLIC_SUPABASE_ANON_KEY')
}

// Server client — fresh instance per request.
// Used in Server Components, Route Handlers, Server Actions.
// NEVER imported in client components (server-only guard above enforces this).
export async function createServerSupabaseClient() {
  const cookieStore = await cookies()

  return createServerClient(
    SUPABASE_URL!,
    SUPABASE_ANON_KEY!,
    {
      cookies: {
        getAll() {
          return cookieStore.getAll()
        },
        setAll(cookiesToSet) {
          try {
            cookiesToSet.forEach(({ name, value, options }) =>
              cookieStore.set(name, value, options)
            )
          } catch {
            // Server Component — cookie setting handled by middleware
          }
        },
      },
    }
  )
}

// Admin client with service role — server-only. Bypasses RLS.
// Only used in API routes that require privileged access.
export function createServiceRoleClient() {
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error('[ZENITH] Missing env var: SUPABASE_SERVICE_ROLE_KEY')
  }

  return createClient(
    SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY,
    {
      auth: {
        autoRefreshToken: false,
        persistSession:   false,
      },
    }
  )
}
